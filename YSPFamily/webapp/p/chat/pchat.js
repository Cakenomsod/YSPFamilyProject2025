...
userEmailBtn.textContent = usersData.Email;
  ...
  let usersNumber = usersData.NumberData;

  let OFakename = usersDataO.Fakename;
  let ONumber = usersDataO.Number;

  let companionNumber = usersDataN.Number;
  let companionNickname = usersDataN.Nickname;
...
async function sendMessageWithAutoNumber(text, isImage = false) {
  ...
  const messagesRef = collection(db, "chat", "members", "usersP", usersNumber, "messages");
...
async function loadChat() {
  ...
  const yourRef = collection(db, "chat", "members", "usersP", usersNumber, "messages");
  const partnerRef = collection(db, "chat", "members", "usersN", companionNumber, "messages");
  const ORef = collection(db, "chat", "members", "usersOLD", ONumber, "messages");
  ...
  sorted.forEach((msg) => {
    ...
    div.classList.add(msg.sender === "P" ? "you" : "other");
    ...
    div.innerHTML = `
      <div class="sender-name">
        ${msg.sender === "P" ? usersFakename : 
            msg.sender === "O" ? OFakename : 
            companionNickname}
        </div>      
        ${content}
  ...
  onSnapshot(partnerRef, (snapshot) => {
    snapshot.forEach((doc) => {
      const data = doc.data();

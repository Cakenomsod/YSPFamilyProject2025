...
userEmailBtn.textContent = usersData.Email;
  ...
  let usersNickname = usersData.Nickname;

  let OFakename = usersDataO.Fakename;
  let ONumber = usersDataO.Number;

  let compaFakename = usersDataP.Fakename;
  let compaNumber = usersDataP.NumberData;
...
async function sendMessageWithAutoNumber(text, isImage = false) {
  ...
  const messagesRef = collection(db, "chat", "members", "usersN", usersNumber, "messages");
...
async function loadChat() {
  ...
  const yourRef = collection(db, "chat", "members", "usersN", usersNumber, "messages");
  const partnerRef = collection(db, "chat", "members", "usersP", compaNumber, "messages");
  ...
  sorted.forEach((msg) => {
    ...
    div.classList.add(msg.sender === "N" ? "you" : "other");
